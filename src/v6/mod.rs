...
pub struct V6TrackInfo {
    author: String,
    last_modified: Option<u32>,
}
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct V6Track {
    pub metadata: V6TrackMetadata,
    pub parts: Vec<V6Part>,
}
#[derive(Debug, PartialEq, Eq, Clone, Copy)]
pub struct V6TrackMetadata {
    pub env: V6Environment,
    pub sun_dir: u8,
...
impl Track for V6Track {
    type Part = V6Part;

    type Metadata = V6TrackMetadata;
    ...
    fn decode_meta(data: &[u8], offset: &mut usize) -> Option<Self::Metadata> {
        let env = V6Environment::try_from(read_u8(data, offset)?).ok()?;
...
impl Block for V6Block {
    ...
    fn decode(
        ...
        *offset += z_bytes as usize;
        ...
        let rot_dir = read_u8(data, offset)?;
        let rotation = rot_dir & 3;
        if rotation > 3 {
            return None;
        }
        let dir = V6Direction::try_from((rot_dir >> 2) & 7).ok()?;
    ...
    fn encode(&self, data: &mut Vec<u8>, meta: <Self::Track as Track>::Metadata) {
        ...
        data.push(self.rotation & 3 | (self.dir as u8 & 7) << 2);
